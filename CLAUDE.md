...
- `iwlwifi-enable-320mhz.patch` — forces `no_320 = false` in the Intel iwlwifi driver to enable 320 MHz EHT channels regardless of PCIe link speed
...
Patches are applied with `patch -p1`. Patches that fail silently (already applied) are handled by checking exit codes.
